...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!modal) {
    ...
    modal.className =
      "fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-black bg-opacity-50 hidden modal-backdrop";
    modal.innerHTML = `
      <div class="modal-glass rounded-3xl shadow-2xl max-w-4xl w-full max-h-[90vh] mx-4 relative animate-fade-in overflow-hidden">
        <div class="flex justify-between items-center p-5 border-b border-white/15">
          <div class="flex-1"></div>
          <button id="resume-modal-close" class="close-button w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 flex-shrink-0 mr-4">&times;</button>
        </div>
  ...
  const closeBtn =
    document.getElementById("resume-modal-close") ||
    modal.querySelector("#resume-modal-close");
